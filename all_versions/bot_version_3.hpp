...
//Upgrading to alpha-beta in order to process moves faster
//Status: Good on 10x10 but quite bad on 40x40 if the time is low
//2024/08/06
namespace Player_1 {
	struct Distance_Map {
...
int Player_1::Search(int depth, int player_x, int player_y, int enemy_x, int enemy_y, int stored_delta, int alpha, int beta) {
	...
	for (int T = 0; T < 4; ++T) {
		...
		int coeff = -steps_count * steps_count * steps_count;
