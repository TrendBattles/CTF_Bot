...
//Changing to suit alpha-beta pruning
namespace Player_1 {
	struct Distance_Map {
	...
	int Search(int depth, int player_x, int player_y, int enemy_x, int enemy_y, int stored_delta);
	...
	std::pair <int, int> Turn() {
		...
		*/
		...
		int EV = Search(std::min(max_depth, time_left), cell_x - 1, cell_y - 1, opponent_x - 1, opponent_y - 1, 0);
...
int Player_1::Search(int depth, int player_x, int player_y, int enemy_x, int enemy_y, int stored_delta) {
	if (depth == 0) {
	...
	if (analysis_flag[player_x][player_y] == false) {
		...
		int coeff = board[player_x][player_y] * N * 2 / std::max(1, origin_distance) + steps_count * steps_count * steps_count;
		int delta_EV = -Search(depth - 1, enemy_x, enemy_y, player_x, player_y, -(stored_delta + coeff));
	...
	for (int T = 0; T < 4; ++T) {
		...
		int coeff = -steps_count * steps_count;
		int delta_EV = -Search(depth - 1, enemy_x, enemy_y, new_x, new_y, -(stored_delta + coeff));
		
		/* if (debugging) {
			std::cerr << "DEBUG " << delta_EV << ' ' << new_x << ' ' << new_y << '\n';
		} */
		
		if (max_delta >= delta_EV) continue;
