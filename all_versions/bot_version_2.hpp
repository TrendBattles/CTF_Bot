...
std::tuple <int, int, int> Player_1::Search(int depth, int player_x, int player_y, int enemy_x, int enemy_y) {
	if (depth == 0) {
		return std::make_tuple(Estimation(player_x, player_y, enemy_x, enemy_y), -1, -1);
	}
	...
	for (int T = 0; T < 4; ++T) {
		...
		int delta_EV = -std::get <0> (Search(depth - 1, enemy_x, enemy_y, new_x, new_y)) - steps_count * steps_count;
