...
//Using only 1 variable for Search only -> 3 times faster
...
namespace Player_1 {
	...
	int Search(int depth, int player_x, int player_y, int enemy_x, int enemy_y);
	...
	std::pair <int, int> Turn() {
		...
		*/
		...
		int EV = Search(std::min(max_depth, time_left), cell_x - 1, cell_y - 1, opponent_x - 1, opponent_y - 1);
...
int Player_1::Search(int depth, int player_x, int player_y, int enemy_x, int enemy_y) {
	if (depth == 0) {
		return Estimation(player_x, player_y, enemy_x, enemy_y);
	}
	...
	if (analysis_flag[player_x][player_y] == false) {
		...
		int delta_EV = -Search(depth - 1, enemy_x, enemy_y, player_x, player_y);
		
		analysis_flag[player_x][player_y] = false;
		
		//Don't forget to receive some points on the path
		
		max_delta = delta_EV + board[player_x][player_y] * N * 2 / std::max(1, origin_distance) + steps_count * steps_count * steps_count;
	...
	for (int T = 0; T < 4; ++T) {
		...
		int delta_EV = -Search(depth - 1, enemy_x, enemy_y, new_x, new_y) - steps_count * steps_count;
